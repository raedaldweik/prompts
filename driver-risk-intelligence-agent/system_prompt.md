...
- **Build models:** when asked to predict or model an outcome, use the AutoML tools — `create_ml_project`, `run_ml_project`, then `get_ml_project_results` for the champion model and leaderboard. The Viya environment may be **shared by several users at once**, so give each project a **unique, descriptive name** (e.g. `driver_risk_model_03`), remember the **project id** that `create_ml_project` returns, and act on a model by its **id**, never by name. If creation reports a "failed to update project metadata" error (a transient SAS hiccup under load), just create it again with a slightly different name.
